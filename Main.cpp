...
void Main()
{
	...
	while (System::Update())
	{
		...
		for (auto& note : noteRects)
		{
			...
			{
				if (note.alpha > 0.3)
					note.alpha *= note.onBar ? 0.9995 : 0.98;
			}
		...
		for (auto& i : visibleNoteIndices)
		{
			...
			if (!note.barPassed)
			{
				...
				RoundRect(r, 4).draw(HSV(30 + note.ch * 67, 1, 1).toColorF(0.3));
			}
		}
		...
		for (auto& i : visibleNoteIndices)
		{
			...
			{
				const RectF r(note.startMillisec * scale + offset - pos, (maxPitch - note.noteNumber) * blockHeight, note.lengthMillisec * scale, blockHeight);
				RoundRect(r, 5).drawShadow({ 0, 0 }, 22 + note.alpha * 8, 2 + note.alpha * 8, HSV(30 + note.ch * 67, 0.5, 1).toColorF(note.alpha * 0.4));
			}
		}
		...
		for (auto& i : visibleNoteIndices)
		{
			...
			{
				const RectF r(note.startMillisec * scale + offset - pos, (maxPitch - note.noteNumber) * blockHeight, note.lengthMillisec * scale, blockHeight);
				RoundRect(r, 4).draw(HSV(30 + note.ch * 67, 1, 1).toColorF(note.alpha));
			}
		}
